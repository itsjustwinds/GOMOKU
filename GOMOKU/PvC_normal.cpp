...
void load_PvC_normal(int dd[100][100], int chr[100][100], point pre[100][100], point &now, point &Last, int &turn) {
	ifstream Fin("PvC_normal_data.txt");
	for (int i = 0; i < 100; ++i)
		for (int j = 0; j < 100; ++j)
			Fin >> dd[i][j];
	for (int i = 0; i < 100; ++i)
...
void save_PvC_normal(int dd[100][100], int chr[100][100], point pre[100][100], point &now, point &Last, int &turn) {
	ofstream Fout("PvC_normal_data.txt");
	for (int i = 0; i < 100; ++i) {
		for (int j = 0; j < 100; ++j)
			Fout << dd[i][j] << " ";
		Fout << "\n";
...
int PvC_normal(int saved_game, char player1, char player2) {
	...
	int chr[100][100];
	int done = 0;
	reset_data_PvC_normal(pos, dd, chr, pre, now, Last, turn);
	if (saved_game) load_PvC_normal(dd, chr, pre, now, Last, turn);
	clrscr();
	int dem = 1;
	while (1) {
		hide_pointer();
		...
		if (TMP) {
			if (TMP == 1)statistics(0, 1, 0, 2);
			else statistics(0, 0, 1, 2);
			return 1;
		}
		if (turn == 2) {
			point result;
		...
		while (1) {
			...
			if (tmp != -1) {
				...
				if (tmp == F5) {
					save_PvC_normal(dd, chr, pre, now, Last, turn);
					continue;
				}
				if (tmp == 'u') {
	...
	while (1) {
		...
		while (1) {
			...
			if (tmp != -1) {
				...
				if (tmp == 13) {
					if (y == 15) {
						return PvC_normal(0, player1, player2);
					}
					if (y == 16) {
						return PvC_normal(1, player1, player2);
					}
				}
			}
			else Sleep(1);
